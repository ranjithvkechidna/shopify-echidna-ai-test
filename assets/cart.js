...
class CartItems extends HTMLElement {
  constructor() {
    ...
    this.submitBtnEl = document.querySelector(`button[form="${this.formEl?.id}"]`);
    this.submitBtnEl && this.submitBtnEl.addEventListener('click', this.onCartSubmit.bind(this));
  ...
  onCartSubmit(event) {
    this.clearValidity(this.formEl);
 ...
 updateQuantity(line, quantity, name, target) {
  ...
  fetch('/cart.js')
    ...
    .then(async (cart) => {
      ...
      const groupKey = currentItem.properties?._group;
      const type = currentItem.properties?._type;
      ...
      if (type === "main") {
        ...
        if (quantity === 0) {
          ...
          updates[currentItem.key] = quantity;
        }
      }
    ...
    .then(parsedState => {
      ...
      FoxTheme.pubsub.publish(
        FoxTheme.pubsub.PUB_SUB_EVENTS.cartUpdate,
        { cart: parsedState, target, line, name }
      );
