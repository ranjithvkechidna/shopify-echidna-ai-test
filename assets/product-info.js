    ...
    class ProductInfo extends HTMLElement {
      ...
      handleBlocksPosition() {
        ...
        blocks.forEach((block) => {
          ...
          if (isMobile) {
            ...
            if (isMoveToMedia) {
              block.innerHTML = blockPlacement.innerHTML;
              blockPlacement.innerHTML = '';
              block.removeAttribute('data-moved-below-media');
            }
          } else {
            blockPlacement.innerHTML = block.innerHTML;
            block.innerHTML = '';
      ...
      handleUpdateProductInfo(productUrl, viewMode) {
        return (html) => {
          ...
          updateSourceFromDestination('WarrantyBlock', ({ classList }) => classList.contains('hidden'));
          ...
          this.syncPurchaseControlsVisibility(html);
          this.syncVariantNotPurchasableClass(html);
      ...
      setUnavailable() {
        ...
        const innerForm = document.getElementById(`product-form-${this.dataset.section}`);
        const productFormWrapper = innerForm?.closest('.product-form');
        if (productFormWrapper?.dataset.hidePurchaseWhenSoldout === 'true') {
          productFormWrapper.classList.remove('product-form--hide-purchase-controls');
        }

        productFormWrapper?.classList.add('product-form--variant-not-purchasable');

        const warrantyWrap = document.getElementById(`WarrantyBlock-${this.dataset.section}`);
        warrantyWrap?.classList.add('hidden');
        warrantyWrap?.querySelectorAll('input[name="warranty"]').forEach((el) => {
          el.checked = false;
          el.dataset.checked = 'false';
          el.classList.add('is-unchecked');
        });
      }

      /**
       * Sync .product-form--hide-purchase-controls from refreshed section HTML (sold-out-only), not from the submit
       * button disabled state, so invalid option combinations stay visible with disabled controls.
       */
      syncPurchaseControlsVisibility(parsedDocument) {
        const innerForm = document.getElementById(`product-form-${this.dataset.section}`);
        const wrapper = innerForm?.closest('.product-form');
        if (!wrapper || wrapper.dataset.hidePurchaseWhenSoldout !== 'true') return;

        const freshInner = parsedDocument.getElementById(`product-form-${this.dataset.section}`);
        const freshWrapper = freshInner?.closest('.product-form');
        const shouldHide = freshWrapper?.classList.contains('product-form--hide-purchase-controls') ?? false;
        wrapper.classList.toggle('product-form--hide-purchase-controls', shouldHide);
      }

      syncVariantNotPurchasableClass(parsedDocument) {
        const innerForm = document.getElementById(`product-form-${this.dataset.section}`);
        const wrapper = innerForm?.closest('.product-form');
        if (!wrapper) return;
        ...
        const notPurchasable =
          freshWrapper?.classList.contains('product-form--variant-not-purchasable') ?? false;
        wrapper.classList.toggle('product-form--variant-not-purchasable', notPurchasable);
      }
