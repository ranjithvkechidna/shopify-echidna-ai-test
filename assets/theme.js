...
FoxTheme.config = {
  ...
  isTouch: (() => {
    ...
    return hasTouch && !hasPointerFine;
  })(),
...
const zoomWrapper = document.querySelector('.zoom-hover-enabled');

if (zoomWrapper) {
  zoomWrapper.querySelectorAll('.product__media-item-container').forEach(container => {
...
/* Add Warranty Product in cart-drawer */
...
document.addEventListener("click", async function (e) {
  ...
  const productTitle = form.querySelector('[name="product_title"]')?.value || "Product";

  const warrantyInput = document.querySelector('input[name="warranty"]:checked');
  const warrantyId = warrantyInput ? Number(warrantyInput.value) : null;
  ...
  try {
    ...
    if (warrantyId) {
      cart.items.forEach(item => {
        if (
          item.id === variantId &&
          item.properties?._type === "main"
        ) {
          const group = item.properties?._group;

          const hasWarranty = cart.items.some(w =>
            w.properties?._group === group &&
            w.id === warrantyId &&
            w.properties?._type === "warranty"
          );
    ...
    if (existingGroup) {
      ...
      cart.items.forEach(item => {
        if (item.properties?._group === existingGroup) {
          updates[item.key] = item.quantity + quantity;
        }
      });
...
/* Remove Warranty Product in cart-drawer */
...
document.addEventListener("click", async function (e) {
  ...
  const cartItem = document.querySelector(`[data-key="${itemKey}"]`);

  const spinner = cartItem?.querySelector('.loading__spinner');
  const closeIcon = cartItem?.querySelector('.icon-close');
  ...
  try {
    ...
    const groupKey = clickedItem.properties?._group;
    const updates = {};

    const isWarranty = clickedItem.properties?._type === "warranty";

    if (groupKey && isWarranty) {
      updates[itemKey] = 0;

      const mainItem = cart.items.find(item =>
        item.properties?._group === groupKey &&
        item.properties?._type === "main"
      );

      if (mainItem) {
        updates[mainItem.key] = 0;
      ...
      cart.items.forEach(item => {
        if (item.properties?._group === groupKey) {
          updates[item.key] = 0;
        }
      });
...
/* Quntity Update Warranty Product in cart-drawer */
...
document.addEventListener("change", function (e) {
  ...
  setTimeout(async () => {
    try {
      ...
      const changedItem = cart.items.find(i => i.key === key);
      if (!changedItem) return;

      if (changedItem.properties && changedItem.properties._type !== "main") return;

      const groupKey = changedItem.properties?._group;
      const updates = {};

      cart.items.forEach(item => {
        if (
          item.properties?._group === groupKey &&
          item.properties?._type === "warranty"
        ) {
...
/* Warranty product input Unchecked */

document.addEventListener('click', function (e) {
  const input = e.target.closest('input[name="warranty"]');
  if (!input) return;
