class PredictiveSearch extends HTMLElement {
  constructor() {
    ...
    /** Whether `.toggle-navigation-button` had `is-show` before header search opened (desktop collapse nav). */
    this._savedNavToggleShow = null;

    document.addEventListener('menu-drawer:open', () => {
      this.classList.remove('predictive-search-open');
      document.body.classList.remove('search-open');
      this.restoreNavToggleAfterSearch();
    });
    ...
    this.searchCloseBtn = document.querySelector('.header__search-close');
  ...
  onFocus(event) {
    ...
    if (this.isHeaderSearch && !this.searchRecommendationEmpty) {
      ...
      this.rememberNavToggleAndHideForSearch();
    }
  ...
  rememberNavToggleAndHideForSearch() {
    if (!this.toggleNavigationButton) return;
    if (this._savedNavToggleShow === null) {
      this._savedNavToggleShow = this.toggleNavigationButton.classList.contains('is-show');
    }
    this.toggleNavigationButton.classList.remove('is-show');
  }

  restoreNavToggleAfterSearch() {
    if (!this.toggleNavigationButton) return;
    if (this._savedNavToggleShow !== null) {
      this.toggleNavigationButton.classList.toggle('is-show', this._savedNavToggleShow);
    }
    this._savedNavToggleShow = null;
  }
